...
const Centerarea = () => {
  ...
  const file_names = useSelector((state) => state.file_names);
      ...
      const postToBackend = async () => {
        ...
        await axios
          ...
          .then((res) => {
            ...
            dispatch({ type: "file_names", payload: [] });
          })
          .catch((err) => {
  ...
  useEffect(() => {
    let file_names = [...docFileName, ...imageFileName, ...videoFileName];
    if (file_names.length > 5) {
      alert("Max 5 files allowed");
      setfileerr(true);
    } else {
      dispatch({ type: "file_names", payload: file_names });
      setfileerr(false);
    }
  }, [docFileName, imageFileName, videoFileName, dispatch]);
  return (
    <Fragment>
      <section
